...
const DataTablesHook = (reference: string) => {
  ...
  }, [reference, userData, workAreas, selectReport, metadata]);
  ...
  const filterByDate = (data: any[], startDate: string, endDate: string) => {
    ...
    return data.filter((item: any) => {
      ...
      } else if (item?.viewsDate) {
        const formatDate = item?.viewsDate.split("/").reverse().join("-");
        itemTimestamp = new Date(formatDate).getTime();
      }
  ...
  const filterBySearch = (data: any[], value: string, reference: string) => {
    ...
    //console.log("datos:", data, value, reference);
    return data.filter((item: any) => {
  ...
  const handleSearchAndFilter = async (e: any) => {
    ...
    //console.log("referencia", currentData);
    setDataTable(currentData);
  };
